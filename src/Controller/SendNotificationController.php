...
class SendNotificationController implements RequestHandlerInterface
{
    ...
    public function __construct(UserRepository $users, NotificationSyncer $notificationSyncer)
    {
        $this->users = $users;
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $translator = app(Translator::class); // Replaced `resolve()` with `app()`
        ...
        if ($groupIds) {
            ...
            foreach ($userQuery->pluck('id') as $userId) {
                $allUserIds[] = $userId;
            }
        }
